...
const Navbar = () => {
    ...
    return (
        <div ref={refdiv} className='fixed h-[18vh] transition-all duration-600 ease-in py-[3vh] px-[5vw] z-[99999999] bg-zinc-900/10  w-full text-[14px] flex justify-between items-center overflow-x-hidden '>
            ...
            <div className='md:flex items-center gap-2 hidden'>
                {
                    username ?
                        ...
                        <div className='relative'>
                            ...
                            <h1 onClick={() => setlogoutToggle(!logoutToggle)} className='text-[#cccccc] cursor-pointer text-xl bg-zinc-800 font-semibold px-4 py-2 rounded-full '>{username.trim().charAt(0)}</h1>
                            {logoutToggle &&
                                <Link href='/logout' className='text-red-400 absolute top-[100%] font-[Neue] h-6 rounded-md flex justify-center items-center font-semibold px-4 py-4 text-sm'>logout</Link>
                            }
                        </div> : <>
            ...
            <div className='md:hidden relative flex  h-12 w-12 gap-2 z-[9999999] ' onClick={handleToggle} >
            ...
            <div className={`w-full h-screen bg-transparent backdrop-blur-xl top-0 fixed px-[8vw] py-[4vh] transition-all ease-in-out duration-500 ${toggle ? "left-0" : " left-[100%] "}`}>
                {
                    username &&
                ...
                <h1 className='text-4xl font-[Geist] tracking-tighter block pb-4 text-zinc-100'>Menu</h1>
                {
                    username ?
                    (
                        ["Home", "RoadMaps", "About", "Community", "Resume Builder (Beta)", "logout"].map((item, i) => (
